import { MAX_UPLOAD_IMAGE_SIZE, MAX_UPLOAD_IMAGE_SIZE_IN_MB, applicationName } from "@/app-config";
import {
  createUser,
  deleteUser,
  getUserByEmail,
...
export async function registerUserUseCase(email: string, password: string) {
  ...
  await sendEmail(email, `Verify your email for ${applicationName}`, <VerifyEmail token={token} />);
...
export async function updateProfileImageUseCase(file: File, userId: UserId) {
  ...
  if (file.size > MAX_UPLOAD_IMAGE_SIZE) {
    throw new PublicError(`File size should be less than ${MAX_UPLOAD_IMAGE_SIZE_IN_MB}MB.`);
  }
...
export async function updateProfileNameUseCase(userId: UserId, displayName: string) {
