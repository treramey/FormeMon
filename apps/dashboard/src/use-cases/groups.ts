...
export async function createGroupUseCase(
  ...
  if (numberOfGroups >= (plan === "premium" ? MAX_GROUP_PREMIUM_LIMIT : MAX_GROUP_LIMIT)) {
...
export async function getPublicGroupsByUserIdUseCase(userId: UserId) {
  return [...(await getPublicGroupsByUser(userId)), ...(await getPublicGroupsByMembership(userId))];
}
...
export async function getGroupByIdUseCase(authenticatedUser: UserSession, groupId: GroupId) {
