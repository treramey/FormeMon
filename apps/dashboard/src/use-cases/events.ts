...
export async function createEventUseCase(
  ...
  if (eventImage) {
    ...
    if (eventImage.size > MAX_UPLOAD_IMAGE_SIZE) {
      throw new PublicError(`File size too large. Max size is ${MAX_UPLOAD_IMAGE_SIZE_IN_MB}MB`);
    }
  }
...
export async function editEventUseCase(
  ...
  if (eventImage) {
    const imageId = createUUID();
    await uploadFileToBucket(eventImage, getGroupImageKey(event.groupId, imageId));
    await updateEvent(event.id, {
      imageId,
    });
  }
