...
export async function followUserUseCase(authenticatedUser: UserSession, foreignUserId: UserId) {
  await createFollow({ userId: authenticatedUser.id, foreignUserId });
}

export async function unfollowUserUseCase(authenticatedUser: UserSession, foreignUserId: UserId) {
