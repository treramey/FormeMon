...
export async function joinGroupUseCase(authenticatedUser: UserSession, groupId: number) {
  const membership = await getMembership(authenticatedUser.id, groupId);
  if (membership) {
...
export async function leaveGroupUseCase(authenticatedUser: UserSession, groupId: number) {
  const membership = await getMembership(authenticatedUser.id, groupId);
  if (!membership) {
...
export async function switchMemberRoleUseCase(
  authenticatedUser: UserSession,
  { userId, groupId, role }: { userId: UserId; groupId: number; role: "admin" | "member" },
) {
  const membership = await getMembership(userId, groupId);
  if (!membership) {
    throw new PublicError("User is not a member of this group");
