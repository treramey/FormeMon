...
export async function deleteReplyUseCase(
  ...
  const hasAccess = await isAdminOrOwnerOfGroup(authenticatedUser, post.groupId);
...
export async function updateReplyUseCase(
  ...
  const replyAccess = await hasAccessToMutateReply(authenticatedUser, reply.replyId);
