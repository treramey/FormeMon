...
import { formGroupStyles, pageTitleStyles, pageWrapperStyles } from "@/styles/common";
      ...
      <PageHeader>
        <div className="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center">
          <div className="flex flex-col gap-4 sm:gap-8 flex-grow">
            <h1 className={`${pageTitleStyles} text-2xl sm:text-3xl md:text-4xl`}>Browse Groups</h1>
              ...
              <div className={formGroupStyles}>
                <div className="flex flex-col sm:flex-row gap-2 items-stretch sm:items-center">
                  <div className="flex relative w-full sm:max-w-md">
                    ...
                    />
                    {search && (
                      <Button size="icon" variant="link" className="absolute right-1" asChild>
                        <Link href={`/browse`}>
                          <XIcon />
                        </Link>
                      </Button>
...
async function GroupsList({ search, page }: { search?: string; page: number }) {
  const { data, perPage, total } = await searchPublicGroupsUseCase(search ?? "", page);
  ...
  if (data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 gap-8 dark:bg-slate-900 rounded-xl">
        ...
        />
        <h2 className="text-xl sm:text-2xl text-center">No groups matching your search</h2>
      </div>
    );
  }
  return (
    <>
      ...
      <GroupPagination search={search ?? ""} page={page} totalPages={Math.ceil(total / perPage)} />
    </>
  );
}
