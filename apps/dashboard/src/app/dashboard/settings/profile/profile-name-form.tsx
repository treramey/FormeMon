...
export function ProfileNameForm({ profileName }: { profileName: string }) {
  ...
  const { execute: updateProfileName, isPending } = useServerAction(updateProfileNameAction, {
    onSuccess: () => {
      toast({
        title: "Name Updated",
  ...
  const onSubmit: SubmitHandler<z.infer<typeof updateProfileNameSchema>> = (values) => {
    updateProfileName(values);
  };

  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 flex gap-2 flex-1">
        <FormField
