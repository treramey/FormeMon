...
import { createEventUseCase, deleteEventUseCase, editEventUseCase } from "@/use-cases/events";
...
export const createEventAction = authenticatedAction
  ...
  .handler(
    async ({ input: { groupId, name, description, startsOn, fileWrapper }, ctx: { user } }) => {
...
export const editEventAction = authenticatedAction
  ...
  .handler(
    async ({ input: { eventId, name, description, startsOn, fileWrapper }, ctx: { user } }) => {
      const eventImage = fileWrapper.get("file") as File;
