    ...
    <div className={cn(cardStyles, "border p-4 flex flex-col gap-4")}>
      <div className="flex justify-between">
        <h3 className="text-xl font-bold">
          <Link className={linkStyles} href={`/dashboard/groups/${post.groupId}/posts/${post.id}`}>
            {post.title}
          </Link>
        </h3>
      </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-4 sm:gap-0">
        ...
        {canDeletePost ? (
          <Button asChild className="w-full sm:w-fit">
            <Link href={`/dashboard/groups/${post.groupId}/posts/${post.id}`}>Manage post...</Link>
          </Button>
        ) : (
          <Button asChild className="w-full sm:w-fit" variant={"secondary"}>
            <Link href={`/dashboard/groups/${post.groupId}/posts/${post.id}`}>Read post...</Link>
          </Button>
        )}
      </div>
    </div>
