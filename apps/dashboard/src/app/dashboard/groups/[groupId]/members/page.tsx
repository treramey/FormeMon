...
function MemberCard({
  ...
  return (
    <div key={member.userId} className="flex items-center gap-4">
      <div className={cn(cardStyles, "flex gap-4 hover:underline items-center p-4")}>
        <Avatar>
        ...
        {showActions && (
          <MemberCardActions userRole={member.role} groupId={groupId} userId={member.userId} />
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap gap-4">
        {owners.map((member) => (
          <MemberCard showActions={false} groupId={groupId} key={member.userId} member={member} />
        ))}
      </div>
